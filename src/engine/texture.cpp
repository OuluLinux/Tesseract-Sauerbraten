...
SDL_Surface *loadsurface(const char *name)
{
    ...
    if(z)
    {
        ...
        if(rw)
        {
            ...
            if(!s && verbose) 
            {
                logoutf("Verbose: IMG_LoadTyped_RW failed for %s: %s", name, IMG_GetError());
#ifdef WIN32
                logoutf(" (LastError: %d)", (int)GetLastError());
#endif
            }
            SDL_FreeRW(rw);
        }
    ...
    if(!s) 
    {
        ...
        if(!s && verbose) 
        {
            logoutf("Verbose: IMG_Load failed for %s (filename: %s): %s", name, filename, IMG_GetError());
#ifdef WIN32
            logoutf(" (LastError: %d)", (int)GetLastError());
#endif
        }
...
{
    string pname;
    copystring(pname, makerelpath("packages/sky", name));
    path(pname);
    Texture *t = NULL;
    if(!strchr(pname, '*'))
    {
